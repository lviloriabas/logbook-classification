...
5. Confirme el nombre del batch y el **Máximo por batch**. No hay dos valores ni un valor fijo en el código: **Máximo por batch** y **Repartir en** leen y guardan siempre la última cantidad elegida. Deje **Sesión** vacío. En el primer acceso, complete el inicio de sesión y el segundo factor en Edge.
6. Pulse **Subir a AirVault**. Cada `Empty-Batch` se identifica leyendo el `Batch Name` de su primera página, se renombra y se confirma antes de enviar el siguiente PDF; no hace falta corregirlo ni pulsar **Revisar en AirVault**. Si AirVault demora el nombre, el programa lo reintenta y continúa solo al confirmarlo. Si un batch no aparece después de 30 minutos, vuelve a consultar, reenvía automáticamente una sola vez solo el ausente y sigue confirmando su ID y título sin crear más copias. Puede abrir otra ejecución en otra ventana; ambas avanzan simultáneamente.
