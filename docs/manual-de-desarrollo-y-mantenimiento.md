...
El OCR y `fleet.json` solo admiten `HP-XXXXCMP` o `HP-XXXXWWP`. El adaptador de AirVault también acepta `HK-XXXX` con sufijo opcional al importar CSV o caché. `normalizar_matricula()` aplica además los alias de `_ALIAS_PICKLIST`, los aviones que AirVault tiene con otro sufijo: `HP-1522WWP` se escribe como `HP-1522CMP`, que es el valor de su picklist. La traducción vale para lo que se envía y para lo que se lee de vuelta al contrastar una página remota, y no toca el CSV, `fleet.json` ni los nombres de archivo.
