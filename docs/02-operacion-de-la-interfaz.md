...
**Depurar**, junto a **Exportar**, quita de la ejecución las páginas repetidas o en blanco. El cuadro dice cuántas hay de cada clase antes de borrar nada; marque las que correspondan y pulse **Eliminar**. De cada bitácora repetida se va una sola aparición, la más nueva: puede elegir cuál conservar, pero no marcarlas todas, porque la ejecución se quedaría sin esa bitácora. Se reescriben el CSV mínimo, el CSV completo, el JSON y `stats.json` sin ellas, y la tabla y la vista previa se rehacen en el acto. Los PDF se generan al exportar, así que salen ya sin esas páginas. El botón espera a que la ejecución esté guardada y a que no haya ninguna escritura en curso; tras una ejecución cancelada no está disponible.
